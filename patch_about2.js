...
const PAGE_T = {
  vi: {
    ...
    desc: "Công thức toán học và kiến trúc của cỗ máy tạo ý tưởng SPIN//BUILD."
  },
  en: {
    ...
    desc: "Mathematical formula and architecture of the SPIN//BUILD idea engine."
  }
};
`;
...
content = content.replace(/const BAND_VI: Record<string, string> = \{\n(?:.*\n)*?\};\n/, '');
content = content.replace(/const KEYS: \[string, string\]\[\] = \[\n(?:.*\n)*?\];\n/, '');
content = content.replace(/const LIVE_SOURCES = \[\n(?:.*\n)*?\];\n/, '');
content = content.replace(/LIVE_SOURCES/g, 'SOURCES');
content = content.replace(/BAND_VI\[band.label\]/g, 'BANDS.find(b => b.label === band.label)?.desc');
